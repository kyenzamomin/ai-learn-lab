...
// Expanded static quiz data (same as in your QuizPage for consistency)
const staticQuizzes = [
  // ================================
  // Programming
  // ================================
...
const QuizDetailPage = () => {
  ...
  const quizIdParam = Array.isArray(params.quizId) ? params.quizId[0] : params.quizId;
  const quizId = parseInt(quizIdParam, 10);

  const quiz = staticQuizzes.find((q) => q.id === quizId);

  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [score, setScore] = useState(0);
  const [showResults, setShowResults] = useState(false);
  const [userAnswers, setUserAnswers] = useState<Array<{ question: string; selected: string | null; correct: string }>>([]);

  if (!quiz) {
    return <p className="p-8 text-center text-red-500">No quiz found for ID {quizId}.</p>;
  }
  ...
  const handleNextQuestion = () => {
    ...
    setUserAnswers([...userAnswers, { question: question.question, selected: selectedAnswer, correct: question.correctAnswer }]);
      ...
      {!showResults ? (
        <div className="p-6 border border-gray-300 rounded-lg shadow-md">
          ...
          <button
            ...
            className="mt-4 bg-[#5A3FFF] text-white px-4 py-2 rounded disabled:bg-gray-400"
          >
            {currentQuestionIndex + 1 < quiz.questions.length ? "Next Question" : "See Results"}
          </button>
        </div>
      ) : (
        <div className="p-6 border border-gray-300 rounded-lg shadow-md">
          <h2 className="text-2xl font-bold">Quiz Completed!</h2>
          <p className="mt-2 text-lg">Your Score: {score} / {quiz.questions.length}</p>
          <div className="mt-4">
            <h3 className="text-xl font-semibold">Review Your Answers</h3>
            {userAnswers.map((item, index) => (
              <div key={index} className="mt-3 p-3 border rounded-md">
                <p className="font-semibold">{index + 1}. {item.question}</p>
                <p className={`mt-1 ${item.selected === item.correct ? "text-green-600" : "text-red-600"}`}>
          ...
          <button
            ...
            className="mt-4 bg-[#5A3FFF] text-white px-4 py-2 rounded"
          >
            Restart Quiz
          </button>
          <button
